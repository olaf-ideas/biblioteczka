...
struct Modular {
    ...
    template <class T> Modular(T v = 0) { value = v % MOD; if(value < 0) value += MOD; }
    template <class T> Modular(T a, T b) : value(0) { *this += a; *this /= b; }
    ...
    friend Modular pot(Modular a, long long e) {
        Modular res = 1;
